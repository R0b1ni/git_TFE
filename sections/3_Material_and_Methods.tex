...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Velocity Triangle 2.pdf}
    \caption{Velocities and angles at the inlet and outlet of the turbine, for the mean radius}
    \label{fig:theorical_velocity_triangles}
\end{figure}
...
Then, we will calculate $c_{2 \theta,m}$ velocity at the outlet of the stator. We also know that the stator doesn't produce any work so we only need the velocity triangle of the rotor.
\begin{figure}[H]
    \centering
...
With all the values calculated in the previous sections, we can now start defining the geometry of the blade. We will use a methodology based on Janjua et al. (2013). We can first define the different convention for the different angles and coordinates of the blade, as shown in the following figure:
To insert : blade geometry convention

Knowing that for each radius $D_i = 2 r_i$, we can calculate the following quantities:
\begin{equation}
\left\{
\begin{aligned}
y_{2,i} &= \sin(\beta_{2,i} - 90°) \times \frac{D_i \sin\left(\frac{s_i''}{D} \times \frac{180°}{\pi}\right)}{2 \cos(\beta_{\infty ,i} - \beta_{2,i})}\\
z_{2,i} &= \frac{y_{2,i}}{\tan(\beta_{2,i} - 90°)}\\
...
\left\{
\begin{aligned}
y_{3,i} &= -\sin(\beta_{3,i} - 90°) \times \frac{D_i \sin\left(\frac{s_i''}{D} \times \frac{180°}{\pi}\right)}{2 \cos(\beta_{3,i} - \beta_{\inf,i})}\\
z_{3,i} &= \frac{y_{3,i}}{\tan(\beta_{3,i} - 90°)}\\
x_{3,i} &= \frac{D_i}{2} - D_i\sin^2\left(\frac{\arcsin\left(\frac{2y_{3,i}}{D}\right)}{2}\right)
\end{aligned}
\right.
\end{equation}
...
to do list:
\begin{itemize}
    \item change velocity triangle graphes
    \item add the angles conventions
    \item add the figure for the blade geometry convention
    \item add the table for the angles, velocities, chord and pitch at each radius
    \item change th eequations for the rpm calculation
\end{itemize}
